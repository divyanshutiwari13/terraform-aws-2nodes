...
- The deployment script waits for the public IP to be mounted and then applies SSL.
- PM2 ensures the Next.js app runs in the background and restarts on reboot.
- Certbot is scheduled via cron for automatic certificate renewal.
- Node.js is installed using NVM to ensure compatibility with the Next.js project.
